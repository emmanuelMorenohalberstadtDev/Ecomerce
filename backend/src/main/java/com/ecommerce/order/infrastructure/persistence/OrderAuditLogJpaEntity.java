 ...
 * {@code V001__create_auth_schema.sql} §8) for writes originating from the order context.
 *
 * <p><strong>Deliberate duplicate of inventory's/catalog's {@code AdminAuditLogJpaEntity}</strong>,
 * not a shared import — see {@code order.application.port.AuditLogPort}'s class javadoc for why.
 * Field-for-field identical mapping; independently maintained per the feature brief.
 *
 * <p>{@code ip_address} is not mapped/populated in v1, same known gap as inventory's/catalog's
 * entity.
 *
...
public class OrderAuditLogJpaEntity {
    ...
    OrderAuditLogJpaEntity() {}

    public OrderAuditLogJpaEntity(UUID actorId, String actorEmail, String actionType,
